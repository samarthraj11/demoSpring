...
class MemoryTest {
    ...
    void ValueItem_Value_IsNull_ReturnFalse() {
        assertEquals(memory.calculateResources(new ValueItem(null, ResourceUnit.MB)), false);
    }

    @Test
    void ValueItemValue_IsNegative_Return_False() {
        assertEquals(memory.calculateResources((new ValueItem(-1, ResourceUnit.MB))), false);
    }
